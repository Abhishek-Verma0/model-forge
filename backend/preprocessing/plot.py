...
def render(df, kind, x=None, y=None, hue=None, fmt="png"):
    ...
    if kind == "scatter":
        if not (x and y):
            raise ValueError("Scatter needs both X and Y columns.")
    ...
    elif kind in ("scatter", "density") and len(df) > SAMPLE:
...
def _title(kind, x, y, hue):
    t = f"{kind}: {x} vs {y}" if kind == "scatter" else f"{kind}: {x}"
...
if __name__ == "__main__":
    ...
    for k in KINDS:
        ...
        img, media = render(d, k, x=col, y=("b" if k == "scatter" else None), hue="g")
